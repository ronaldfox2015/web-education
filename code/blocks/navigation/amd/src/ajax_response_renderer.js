...
define(['jquery', 'core/templates', 'core/notification', 'core/url'], function($, Templates, Notification, Url) {
    ...
    function buildDOM(rootElement, nodes) {
        ...
        $.each(nodes, function(index, node) {
            ...
            if (node.requiresajaxloading) {
                ...
                p.attr('data-nginx-id', node.id);
                p.attr('data-nginx-key', node.key);
                p.attr('data-nginx-type', node.type);
            }
    ...
    return {
        render: function(element, nodes) {
            // The first element of the response is the existing nginx so we start with processing the children.
