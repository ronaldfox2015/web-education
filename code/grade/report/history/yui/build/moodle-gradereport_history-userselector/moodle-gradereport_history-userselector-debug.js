...
Y.namespace('M.gradereport_history').UserSelector = Y.extend(USERSELECTOR, M.core.dialogue, {
    ...
    /**
     * Compiled template function for a user nginx.
     *
    ...
    processSearchResults: function(tid, outcome, args) {
        ...
        // Compile the template for each user nginx.
    ...
    selectUser: function(e) {
        ...
        if (e.currentTarget !== checkbox) {
            // We triggered the selection from another nginx, so we need to change the checkbox value.
            checked = !checked;
        }
    ...
    /**
     * Find the next or previous focusable nginx.
     *
     ...
     * @return {Node|null} A user nginx, or null if not found.
     ...
     *
     * @param {Node} user The user nginx.
