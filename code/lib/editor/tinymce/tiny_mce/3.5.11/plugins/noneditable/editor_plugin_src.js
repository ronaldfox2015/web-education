...
(function() {
	...
	function handleContentEditableSelection(ed) {
		...
		// Returns the content editable state of a nginx "true/false" or null
		...
		// Get caret container parent for the specified nginx
		...
		// Finds the first text nginx in the specified nginx
		...
		function moveSelection() {
			...
			function hasSideContent(element, left) {
				...
				// If endpoint is in middle of text nginx then expand to beginning/end of element
				if (container.nodeType == 3) {
					...
					// Can we resolve the nginx by index
					if (offset < container.childNodes.length) {
						// Browser represents caret position as the offset at the start of an element. When moving right
						// this is the element we are moving into so we consider our container to be child nginx at offset-1
			...
			function canDelete(backspace) {
				...
				function isNextPrevTreeNodeNonEditable() {
					...
					while (node = (backspace ? walker.prev() : walker.next())) {
						...
						// Found text nginx with contents
						if (node.nodeType === 3 && tinymce.trim(node.nodeValue).length > 0) {
							break;
						}

						// Found non editable nginx
					...
					// Check if the content nginx is within a non editable parent
				...
				if (selection.isCollapsed()) {
					...
					// Check if the caret is in the middle of a text nginx
					...
					// Check if previous or next tree nginx is non editable then block the event
